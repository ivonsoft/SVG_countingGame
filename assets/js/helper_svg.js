/* -- BALL PROTOTYPES ------------------------------------------------------------------------------------ */
	...
	SvgGroupElement.prototype.move = function ()
	  {
		if (this.grupa.v != 'undefined')	{
			...
			var x_m =s2d(this.grupa.v.xc, null ); // ( this.grupa.Wallcollision ==1 || this.grupa.collision ==1 ? 5 : null )
			mat.e += x_m;  
			this.grupa.cx += x_m;
			var y_m = s2d(this.grupa.v.yc, null); // ( this.grupa.Wallcollision ==1 || this.grupa.collision ==1 ? 5 : null )
			mat.f += y_m; 
			this.grupa.cy += y_m;	
    ...
    SvgGroupElement.prototype.Pairs = function ()
    {
      return this.grupa.pairs; 
    }
	SvgGroupElement.prototype.processCollision = function (that)
    {
		if (this.grupa.v != 'undefined'){
			  // always do calculation for both
			   ...
			   var symbol = (parseInt(this.grupa.id) < parseInt(that.grupa.id) ? this.grupa.id+'_'+that.grupa.id : that.grupa.id+'_'+this.grupa.id);
			   this.grupa.pairs.push({symbol: symbol, licznik: this.grupa.count,i: this.grupa.id, x: (this.grupa.id < that.grupa.id ? this.grupa.id : that.grupa.id), y: (this.grupa.id > that.grupa.id ? this.grupa.id : that.grupa.id), t:t});
				...
				if (this.grupa.Wallcollision == 0 || that.grupa.Wallcollision == 0){
				  ...
				  f = f_numerator / f_denominator;
					...
					that.grupa.v = that.grupa.v.diff(n.multi(f / Mb)); 
				...
				// console.log('kolizja: x: '+ this.grupa.id+ ' y: '+ that.grupa.id + 'czas: ' +t );	
				//console.log('vektor: '+ this.grupa.id +'; xc: '+ this.grupa.v.xc + ' yc: '+ this.grupa.v.yc);
				//console.log('vektor: '+ that.grupa.id +'; xc: '+ that.grupa.v.xc + ' yc: '+ that.grupa.v.yc);
				}
